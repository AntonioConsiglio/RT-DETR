...
STDC_ROOT = Path(__file__).parent.resolve()
STDC1_PRETRAINED = os.path.join(STDC_ROOT,'pretrained_weights/STDCNet813_73.91.tar')
STDC2_PRETRAINED = os.path.join(STDC_ROOT,'pretrained_weights/STDCNet1446_76.47.tar')
...
class STDCBackbone(nn.Module):
    ...
    def init_weight(self, pretrain_model):
        ...
        if isinstance(pretrain_model,bool):
            state_dict = torch.hub.load_state_dict_from_url(self.weight_url)["state_dict"]
        else:
...
class STDC2(STDCBackbone):
    ...
    weight_url = "https://drive.google.com/uc?export=download&id=1Y5belNkq3Dn-EYgSKY-ICiPsN4TZXoXO"
    def __init__(self, base=64, layers=[4,5,3], block_num=4,
                 pretrain_model=STDC2_PRETRAINED,
                 use_conv_last=False, norm_layer=nn.BatchNorm2d,
                 ):
...
class STDC1(STDCBackbone):
    weight_url = "https://drive.google.com/uc?export=download&id=1DFoXcV42zy-apUcMh5P8WhsXMRJofgl8"

    def __init__(self, base=64, layers=[2,2,2], block_num=4,
                 pretrain_model=STDC1_PRETRAINED,
                 use_conv_last=False, norm_layer=nn.BatchNorm2d):
